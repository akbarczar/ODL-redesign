        ...
        try {
            ...
            foreach ($coll as $feed) {
                ...
                try {
                    ...
                    $cron['file']['localeDate'] = $this->_coreDate->date('l Y-m-d H:i:s', strtotime((string) $feed->getSimplegoogleshoppingTime()) + $offset);
                    $cron['file']['gmtDate'] = $this->_coreDate->gmtdate('l Y-m-d H:i:s', (string) $feed->getSimplegoogleshoppingTime());
                    $cron['file']['localeTime'] = $this->_coreDate->timestamp(strtotime((string) $feed->getSimplegoogleshoppingTime()) + $offset);
                    $cron['file']['gmtTime'] = strtotime((string) $feed->getSimplegoogleshoppingTime());
