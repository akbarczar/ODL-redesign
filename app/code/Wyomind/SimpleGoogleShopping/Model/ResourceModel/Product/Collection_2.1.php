...
class Collection extends \Magento\Catalog\Model\ResourceModel\Product\Collection
{
    ...
    public function getGroupedProducts($storeId, $notLike, $concat, $listOfAttributes)
    {
        ...
        $this->getSelect()->joinLeft(['cpl' => $tableCpl], 'cpl.product_id=e.entity_id AND cpl.link_type_id=3', ['child_ids' => 'GROUP_CONCAT(DISTINCT cpl.linked_product_id)'])->joinLeft(['stock' => $tableCsi], 'stock.product_id=e.entity_id', ['qty' => 'qty', 'is_in_stock' => 'is_in_stock', 'manage_stock' => 'manage_stock', 'use_config_manage_stock' => 'use_config_manage_stock', 'backorders' => 'backorders', 'use_config_backorders' => 'use_config_backorders'])->joinLeft(['url' => $tableCur], "url.entity_type='product' AND url.store_id=" . $storeId . $notLike, ["request_path" => $concat . "(DISTINCT request_path)"])->joinLeft(['categories' => $tableCcp], "categories.product_id=e." . $this->_rowId, ['category_id', 'product_id', 'position'])->joinLeft(['categories_index' => $tableCcpi], "categories_index.category_id=categories.category_id AND  categories_index.product_id=categories.product_id AND categories_index.store_id=" . $storeId, ["categories_ids" => "GROUP_CONCAT( DISTINCT categories_index.category_id)"])->group(['cpl.product_id']);
        $grouped = [];
    ...
    protected function getParentProducts($type, $storeId, $notLike, $concat, $listOfAttributes)
    {
        ...
        $this->getSelect()->joinLeft(['cpsl' => $tableCpsl], 'cpsl.parent_id=e.' . $this->_rowId, ['child_ids' => 'GROUP_CONCAT( DISTINCT cpsl.product_id)'])->joinLeft(['stock' => $tableCsi], 'stock.product_id=e.entity_id', ['qty' => 'qty', 'is_in_stock' => 'is_in_stock', 'manage_stock' => 'manage_stock', 'use_config_manage_stock' => 'use_config_manage_stock', 'backorders' => 'backorders', 'use_config_backorders' => 'use_config_backorders'])->joinLeft(['url' => $tableCur], "url.entity_type='product' AND url.store_id=" . $storeId . $notLike, ["request_path" => $concat . "(DISTINCT request_path)"])->joinLeft(['curpc' => $tableCurpc], "url.url_rewrite_id=curpc.url_rewrite_id and e.entity_id = curpc.product_id")->joinLeft(['categories' => $tableCcp], "categories.product_id=e." . $this->_rowId, ['category_id', 'product_id', 'position'])->joinLeft(['categories_index' => $tableCcpi], "categories_index.category_id=categories.category_id AND  categories_index.product_id=categories.product_id AND categories_index.store_id=" . $storeId, ["categories_ids" => "GROUP_CONCAT( DISTINCT categories_index.category_id)"])->group(['cpsl.parent_id']);
        $parent = [];
        foreach ($this as $p) {
