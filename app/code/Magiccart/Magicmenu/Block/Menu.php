        ...
        if($hasChild || $blocks['top'] || $blocks['left'] || $blocks['right'] || $blocks['bottom']) :
            $desktopTmp .= '<div class="level-top-mega">';  /* Wrap Mega */
                $desktopTmp .='<div class="content-mega">';  /*  Content Mega */
                    ...
                    //$desktopTmp .= '<h1 class="testing">hello category is is '. $idTop .'</h1>';

                    // Get the value of the custom attribute
                    $objectManager = \Magento\Framework\App\ObjectManager::getInstance();
                    $category = $objectManager->get('Magento\Catalog\Model\Category'); 
                    $currentCategory = $category->load($idTop);
                   // $meigeeCatSubcontent = $currentCategory->getData('meigee_cat_subcontent');
                    $meigeeCatSubcontent = $this->getValHtml($currentCategory->getData('meigee_cat_subcontent'));
                        $desktopTmp .= $blocks['left'];
                        if($hasChild) :
                            $desktopTmp .= '<ul class="level0 category-item mage-column cat-mega" style="width:100%">';
                            $desktopTmp .= $blocks['left'];
                            ...
                            $counter = 1;
                            $desktopTmp .= $meigeeCatSubcontent;
                             $mobileTmp  .= $meigeeCatSubcontent;
